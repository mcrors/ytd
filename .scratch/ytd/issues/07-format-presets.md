...
- [ ] `Format` type defined (e.g. string enum: `best`, `1080p`, `audio`)
- [ ] `FormatArgs(f Format) []string` maps preset to yt-dlp flags
- [ ] `Downloader.Download()` accepts a `Format` parameter and passes the correct flags
- [ ] Invalid format value rejected with a clear error
- [ ] Unit tests for each preset mapping
